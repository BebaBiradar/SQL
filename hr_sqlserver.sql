...
--i want only those were sum of salary greater than 80000 
--having where
select department_id, sum(salary) as salary from employees
group by department_id having sum(salary)>=80000;

--list only those department where emp count>=5
select department_id,count(*)'EmpCount'from employees group by department_id
having count(*)>=5;


select *from employees where department_id =(select department_id from departments where department_name='It');

--list all the employee  who are working as programmer
